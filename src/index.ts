import { Product, User, Category, Purchase, PurchasesProductsFK, PurchasesWithProducts } from "./models/model";
...
app.post("/users", async (req: Request, res: Response) => {
    try{
        ...
        //query builder
        await db("users").insert({id: id, name: name, email: email, password: password})
...
app.post('/products', async (req: Request, res: Response) => {
    ...
    try{
    const{id, name, price, description, imgURL, quantity} = req.body as Product
    ...
    await db.raw       
        (`
            INSERT INTO products (id, name, price, description, imgURL, category, quantity)
            VALUES ("${id}", "${name}", ${price},"${description}" , "${imgURL}", "${category}", ${quantity}); 
        `) //precisa de aspas para considerar como strin
...
app.post('/purchases', async (req: Request, res: Response) => {
    ...
    try{
    const{id, buyer_id, totalPrice, paid} = req.body as Purchase

    const statusIdBuyer = await db("users").where({id: buyer_id}) 
    //verifica se o userID digitado existe no banco de dados
    if(!statusIdBuyer) {
        res.status(404) 
        throw new Error("id não cadastrado")
    ...
    //     VALUES ("${id}", "${buyerId}", ${totalPrice}, ${paid}); 
    // `) //precisa de aspas para considerar como string

    await db("purchases").insert({id, buyer_id, totalPrice, paid})
...
//Get purchase by id query builder
app.get('/purchases/:id', async (req: Request, res: Response) => {
    try {
        const id = req.params.id
...
//////////////////Purchase + user info
app.get('/purchaseUserInfo/:id', async (req: Request, res: Response) => {
    try{
        const idPurchase = req.params.id

        const purchase: Purchase[] = await db.select("purchases.*", "users.name", "users.email")
...
///////////////purchase + lista de product
//create
app.post('/purchase', async (req: Request, res: Response) => {
    try{
        const{idPurchase, idProduct, quantity} = req.body
        //const [getUserById]: User[] = await db("users").where("id", "=", `${buyerId}`)

        const newPurchaseProducts: PurchasesProductsFK = {
...
//pegar todas as info juntas
app.get('/purchasesProducts/:id', async (req: Request, res: Response) => {
    try{
        const idPurchase = req.params.id
        ...
        const products: Product[] = await db.select("products.*", "purchases_products.quantity")
        .from("purchases_products")
        .leftJoin("products", "purchases_products.product_id", "=", "products.id")
        .where({ "purchases_products.purchase_id" : idPurchase})

        const allInfo = [{...purchase, listOfProducts: products}]  
